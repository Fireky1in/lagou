	...
	<h3 class="header blue lighter smaller">
		...
		<div class="row-fluid  dataTables_wrapper">
			<div class="span12" id="m_search_div">
				...
				<input name="username" value="<?php echo ($map["name"]); ?>" type="text" id="search_name" class="pull-right" placeholder="用户名"/>
			</div>	
		</div>
		</form>
	</h3>
	...
	<table id="data_table" class="table table-striped table-bordered table-hover" style="margin-bottom:0px;">
		...
		<tbody>
			<?php if(is_array($users)): foreach($users as $key=>$val): ?><tr>
					...
					<td><a href="<?php echo U('Admin/CompanyJob/index', array('id'=>$val['id']));?>" class="btn btn-mini btn-success">点击查看职位</a></td>
					<td><?php echo C('company_state')[$val['state']];?></td>
					<td>
