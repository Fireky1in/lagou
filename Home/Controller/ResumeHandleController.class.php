...
class ResumeHandleController extends CompanyBaseController {
	...
	public function audition()
	{
		...
		$comRes = $this->comObj->where(array('id'=>$this->id))->find();
	...
	public function unhandle()
	{
		...
		$hopeObj = D('Hopejob');
		$array = array('女', '男');
		$res = $sendObj->where(array('company_id'=>$this->id, 'state4'=>array('in', '0,3')))->select();
		foreach ($res as &$val) {
		...
		$company = $this->comObj->where(array('id'=>$this->id))->find();

		$resume_total = $sendObj->where(array('company_id'=>$this->id))->count();
		$unhandle_total = $sendObj->where(array('company_id'=>$this->id, 'state4'=>0))->count();
		$audition_total = $sendObj->where(array('company_id'=>$this->id, 'state4'=>1))->count();
		$notPass_total = $sendObj->where(array('company_id'=>$this->id, 'state4'=>2))->count();
	...
	public function showNotpass()
	{
		...
		$hopeObj = D('Hopejob');
		$array = array('女', '男');
		$res = $sendObj->where(array('company_id'=>$this->id, 'state4'=>2))->select();
		$num = $sendObj->where(array('company_id'=>$this->id, 'state4'=>2))->count();
		foreach ($res as &$val) {
			$res2 = $jobObj->where(array('id'=>$val['job_id']))->find();
			$res3 = $resumeObj->where(array('id'=>$val['user_id']))->select();
			foreach ($res3 as $val3) {
	...
	public function showAudition()
	{
		...
		$condition['s.company_id'] = $this->id;
