...
class CompanyJobController extends CompanyBaseController {
	...
	{
		$jobObj = D('Job');
		$job = $jobObj->where(array('company_id'=>$this->id, 'state'=>1))->select();
		$jobnum = $jobObj->where(array('company_id'=>$this->id, 'state'=>1))->count();
	...
	{
		$jobObj = D('Job');
		$job = $jobObj->where(array('company_id'=>$this->id, 'state'=>0))->select();
		$jobnum = $jobObj->where(array('company_id'=>$this->id, 'state'=>0))->count();
